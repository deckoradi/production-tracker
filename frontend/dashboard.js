...
// ============ OBRISI SVE NALOGE - ISPRAVLJEN URL ============
document.getElementById('clearAllBtn')?.addEventListener('click', async () => {
    if (!confirm('⚠️ Ovo će OBRISATI SVE naloge i faze! Nastaviti?')) return;
    ...
    try {
        // ⭐ ISPRAVLJEN URL: /api/clear-orders (ne /api/orders/clear)
        ...
        if (response.ok) {
            if (statusDiv) {
                statusDiv.textContent = '✅ Svi nalozi su obrisani! Sada možeš upload-ovati novi Excel.';
                statusDiv.className = 'success';
            }
