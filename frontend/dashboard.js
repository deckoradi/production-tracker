...
try{
const d=await api('/api/clear-orders',{method:'POST',headers()});
...
try{
const d=await api('/api/clear-all',{method:'POST',headers()});
...
h+=<div class="phase-card"> <h4>Faza ${esc(p.phase)}</h4> <div style="text-align:center; padding:6px 0;"> <div style="font-size:32px;">${emoji}</div> <div style="font-weight:bold; font-size:16px; margin-top:2px;">${label}</div> <div style="font-size:13px; color:#718096; margin-top:2px;">📅 ${dateStr}</div> </div> <div class="phase-buttons" style="justify-content:center; gap:8px; display:flex; flex-wrap:wrap; margin:8px 0;"> <button onclick="updatePhase(${o.id},'${js(p.phase)}','completed')">✅ Urađeno</button> <button onclick="updatePhase(${o.id},'${js(p.phase)}','problem')">⚠️ Problem</button> <button onclick="updatePhase(${o.id},'${js(p.phase)}','pending')">⬜ Reset</button> </div> <textarea style="width:100%; min-height:60px; margin-top:4px;" onblur="saveComment(${o.id},'${js(p.phase)}',this.value)" placeholder="Komentar...">${esc(p.comment||'')}</textarea> </div>;
});
...
async function updatePhase(id,phase,status){const o=orders.find(x=>String(x.id)===String(id)),p=o?.progress.find(x=>String(x.phase)===String(phase));if(!p)return;const old={status.status,comment.comment,updatedAt.updatedAt};p.status=status;p.updatedAt=new Date().toISOString();renderOrders();renderModal(o);try{const d=await api('/api/update-phase',{method:'POST',headers(true),body.stringify({orderId,phase,status,comment.comment||''})});p.updatedAt=d.updatedAt;renderOrders();renderModal(o)}catch(e){Object.assign(p,old);renderOrders();renderModal(o);alert('❌ '+e.message)}}
