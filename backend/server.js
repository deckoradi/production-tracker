...
// ============ UPLOAD - SA TVOJIM NAZIVIMA KOLONA ============
    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const company = findValue(row, [
                'FIRMA', 'ime firme', 'IME FIRME', 'Firma', 'firma', 'Ime firme', 
                'Name', 'name', 'Company', 'company', 'Naziv firme'
            ]);
            
            // ŠIFRA → code
            const code = findValue(row, [
                'ŠIFRA', 'cod artikal', 'COD ARTIKAL', 'Sifra', 'sifra', 
            ...
            const name = findValue(row, [
                'NAZIV', 'naziv artikla', 'NAZIV ARTIKLA', 'Naziv', 'naziv', 
                'Naziv artikla', 'Name', 'name', 'Artikal', 'Proizvod'
            ]);
            
            // NALOG → orderNumber
            const orderNumber = findValue(row, [
                'NALOG', 'broj nalog', 'BROJ NALOG', 'Nalog', 'nalog', 
                'Broj naloga', 'broj naloga', 'BROJ NALOGA',
                'Order', 'order', 'Order Number'
            ]);
            
            // KOLIČINA → quantity
            const quantity = parseInt(findValue(row, [
                'KOLIČINA', 'pari', 'PARI', 'Kolicina', 'kolicina', 
                'QUANTITA', 'Quantity', 'quantity', 'Količina'
            ])) || 0;
            
            // DATUM → deliveryDate
            const deliveryDate = findValue(row, [
                'DATUM', 'datum isporuke', 'DATUM ISPORUKE', 'Datum', 'datum', 
                'Datum isporuke', 'Delivery Date', 'delivery'
            ]);
...
app.get('/api/orders', authenticate, async (req, res) => {
    try {
        ...
        // ============ MAPIRANJE U camelCase ZA FRONTEND ============
