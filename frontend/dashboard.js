...
// ============ RENDER PHASES – BEZ "FAZA" PREFIKSA ============
function renderPhases(order) {
    ...
    phases.forEach(phase => {
        ...
        // ⭐ IZMENA: prikazujemo samo naziv faze, bez "Faza" prefiksa
        html += `
            <div class="phase-card">
                ...
                <div class="phase-buttons">
                    <button onclick="updatePhase(${order.id}, '${escapeJs(phase.phase)}', 'completed')">✅ Završi</button>
                    <button onclick="updatePhase(${order.id}, '${escapeJs(phase.phase)}', 'problem')">⚠️ Problem</button>
                    <button onclick="updatePhase(${order.id}, '${escapeJs(phase.phase)}', 'pending')">⬜ Reset</button>
                </div>
                <div class="phase-comment">
                    <textarea 
                        placeholder="Komentar..." 
                        onchange="updatePhaseComment(${order.id}, '${escapeJs(phase.phase)}', this.value)"
                    >${escapeHtml(phase.comment || '')}</textarea>
                </div>
            </div>
        `;
    });
