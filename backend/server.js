...
try {
  if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
    ...
    console.log('📧 Email transporter: ❌ Nije kreiran (fale podaci)');
  }
} catch (error) {
...
app.post('/api/upload', authenticate, upload.single('file'), (req, res) => {
  ...
  try {
    ...
    console.log('📊 Pronađene kolone u Excel-u:', Object.keys(data[0] || {}));
    ...
    res.json({ 
      message: `✅ Uspešno učitano ${allOrders.length} naloga od ${data.length} redova`,
...
// ============ SEND REPORT - EXCEL FAJL (SAMO DANAŠNJE AKTIVNOSTI) ============
app.post('/api/send-report', authenticate, async (req, res) => {
  try {
    if (!transporter) {
      console.log('❌ Email transporter nije kreiran!');
      return res.status(400).json({ 
        error: 'Email not configured. Add EMAIL_USER and EMAIL_PASS to .env file' 
      });
    }
    ...
    // ======== FILTRIRAJ SAMO NALOGE SA DANAŠNJIM AKTIVNOSTIMA ========
    const activeOrders = [];
    ...
    if (activeOrders.length === 0) {
      ...
      return res.json({ 
        message: '✅ Izveštaj poslat! Nema novih aktivnosti za danas.',
        hasActivity: false
      });
    }
    ...
    dateCell.value = `Datum: ${today} (samo današnje aktivnosti)`;
    dateCell.font = { size: 12, bold: true };
    dateCell.alignment = { horizontal: 'center' };

    // ZAGLAVLJE
    const headers = [
      'Nalog', 'Artikal', 'Šifra', 'Količina', 'Datum isporuke',
    ...
    const mailOptions = {
      ...
      to: email || process.env.ADMIN_EMAIL || 'grupkovic@gmail.com',
      subject: `📊 Dnevni izveštaj - ${req.user.company} - ${today}`,
      text: `Poštovani,\n\nU prilogu vam šaljemo dnevni izveštaj za firmu ${req.user.company} za datum ${today}.\n\nIzveštaj sadrži SAMO naloge sa današnjim aktivnostima (${activeOrders.length} aktivnih naloga).\n\nS poštovanjem,\nProduction Tracker`,
      attachments: [{
    ...
    console.log('✅ Email sa Excel fajlom poslat:', result.messageId);
    console.log(`📊 Aktivnih naloga: ${activeOrders.length}`);

    res.json({ 
      message: `✅ Izveštaj poslat! ${activeOrders.length} aktivnih naloga.`,
      hasActivity: true,
    ...
    res.status(500).json({ 
      error: 'Greška pri slanju email-a: ' + error.message 
    });
  }
});
